...
const margin={top:26,right:18,bottom:38,left:52};
...
function Special({p,w,h,data}:{p:ChartProps;w:number;h:number;data:RecordRow[]}){const key=p.valueKey||'value'; if(p.chartType==='histogram'){const values=data.map(d=>num(d[key]));const [lo,hi]=extent(values), bins=Array.from({length:8},()=>0);values.forEach(v=>bins[Math.min(7,Math.floor((v-lo)/(hi-lo||1)*8))]++);return <Cartesian p={{...p,chartType:'bar',data:{bins:bins.map((v,i)=>({label:Math.round(lo+(hi-lo)*i/8),value:v}))},dataPath:'/bins',categoryKey:'label',valueKey:'value'}} w={w} h={h} data={[] } series={[]}/>}
 ...
 if(p.chartType==='bullet'){const max=num(resolve(p.gaugeMax,p.data),Math.max(num(resolve(p.bulletTarget,p.data)),num(resolve(p.bulletValue,p.data)),1));const v=num(resolve(p.bulletValue,p.data)),t=num(resolve(p.bulletTarget,p.data));return <><rect x={margin.left} y={h/2-18} width={w-margin.left-margin.right} height="36" rx="7" fill="#EAEFF5"/>{(p.thresholds||[]).map((b,i)=><rect key={i} x={scale(num(resolve(b.from,p.data)),[0,max],[margin.left,w-margin.right])} y={h/2-18} width={scale(num(resolve(b.to,p.data))-num(resolve(b.from,p.data)),[0,max],[0,w-margin.left-margin.right])} height="36" fill={statusColor[b.status||'neutral']} opacity=".24"/>)}<rect x={margin.left} y={h/2-8} width={scale(v,[0,max],[0,w-margin.left-margin.right])} height="16" rx="4" fill="#EB0029"/><line x1={scale(t,[0,max],[margin.left,w-margin.right])} x2={scale(t,[0,max],[margin.left,w-margin.right])} y1={h/2-28} y2={h/2+28} stroke="#1C1E21" strokeWidth="3"/></>}
...
export function Chart(p:ChartProps){const [ref,w]=useWidth(),h=p.height||300;const data=rows(p.data,p.dataPath);const title=resolve(p.title,p.data);const series=useMemo(()=>p.series||[],[p.series]);const isRadial=['pie','donut'].includes(p.chartType);return <section ref={ref} className={p.className} style={{background:'#fff',border:'1px solid #E2E6EC',borderRadius:16,padding:18,boxShadow:'0 4px 20px -2px rgba(28,30,33,.06)'}}>{title&&<h3 style={{margin:'0 0 3px',fontSize:18,color:'#1C1E21'}}>{title}</h3>}{resolve(p.subtitle,p.data)&&<p style={{margin:'0 0 12px',fontSize:12,color:'#7A8290'}}>{resolve(p.subtitle,p.data)}</p>}<svg width="100%" height={h} viewBox={`0 0 ${w} ${h}`} role="img" aria-label={title||p.chartType}>{isRadial?<Radial p={p} w={w} h={h} data={data}/>:p.chartType==='gauge'?<Gauge p={p} w={w} h={h}/>:<Special p={p} w={w} h={h} data={data}/>}</svg>{resolve(p.showLegend,p.data)!==false&&series.length>1&&<div style={{display:'flex',gap:12,flexWrap:'wrap',fontSize:12,color:'#4A515E'}}>{series.map((s,i)=><span key={i}><i style={{display:'inline-block',width:8,height:8,borderRadius:8,background:s.color||palette[i],marginRight:5}}/>{resolve(s.name,p.data)||`Serie ${i+1}`}</span>)}</div>}</section>}
