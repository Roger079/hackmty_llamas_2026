...
export const InvestmentSimulatorCard: React.FC<InvestmentSimulatorCardProps> = ({ initialAmount = 25000, initialTermDays = 91, annualRate = '9.1%', estimatedGain, totalMaturity }) => {
  ...
  return (
    <section className="my-3 overflow-hidden rounded-2xl bg-white text-slate-900 shadow-[0_14px_38px_rgba(15,23,42,0.12)]">
      <header className="flex items-center justify-between bg-[#3E4651] px-5 py-4 text-white"><div className="flex items-center gap-2"><Landmark className="h-5 w-5" /><h3 className="text-sm font-bold">Pagaré Banorte</h3></div><span className="rounded-full bg-white/10 px-2.5 py-1 text-[11px] font-bold">Tasa fija {annualRate}</span></header>
      <div className="space-y-5 p-5">
        ...
        <p className="flex items-center gap-2 text-xs text-slate-500"><TrendingUp className="h-4 w-4 text-emerald-600" />Simulación ilustrativa a {initialTermDays} días. El rendimiento final está sujeto a contratación.</p>
      </div>
    </section>
  );
};
