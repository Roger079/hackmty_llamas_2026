  ...
  return (
    <section className="my-3 overflow-hidden rounded-2xl bg-white text-slate-900 shadow-[0_14px_38px_rgba(15,23,42,0.12)]">
      ...
      <div className="space-y-5 p-5">
        <div><label htmlFor="investment-amount" className="text-xs font-bold text-slate-700">Monto a invertir</label><input id="investment-amount" type="range" min="1000" max={Math.max(100000, initialAmount)} step="1000" value={amount} onChange={(event) => setAmount(Number(event.target.value))} className="mt-3 w-full accent-[#EB0029]" /><p className="mt-2 text-2xl font-black tabular-nums">{money(amount)}</p></div>
